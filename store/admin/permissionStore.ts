...
interface PermissionStore extends PermissionStoreState, PermissionStoreActions { }
...
export const usePermissionStore = create<PermissionStore>()((set, get) => ({
    ...
    fetchAll: async () => {
        ...
        try {
            const [permissionsRes, categoriesRes, roleTemplatesRes, adminUsersRes] = await Promise.all([
                supabase.from('permissions_with_category').select('*').order('category_name').order('name'),
                supabase.from('permission_categories_with_stats').select('*').order('order_index'),
                supabase.from('role_templates_with_stats').select('*').order('is_system_role', { ascending: false }).order('name'),
    ...
    revokePermission: async (userId: string, permissionId: string) => {
        try {
            ...
            set({ error: error instanceof Error ? error.message : 'Failed to revoke permission' });
        }
    },

    updateUserPermissions: async (userId: string, permissionIds: string[], grantedBy: string) => {
        try {
            // Deactivate existing permissions
            await supabase.from('user_permissions').update({ is_active: false }).eq('user_id', userId);
            ...
            if (permissionIds.length > 0) {
                ...
                const { error } = await supabase.from('user_permissions').upsert(newPermissions);
                if (error) throw error;
            }

            await Promise.all([get().fetchUserPermissions(userId), get().fetchAdminUsers()]);
        } catch (error) {
            set({ error: error instanceof Error ? error.message : 'Failed to update user permissions' });
        }
    },

    applyRoleTemplate: async (userId: string, roleTemplateId: string, grantedBy: string) => {
        try {
            const template = get().roleTemplates.find(t => t.id === roleTemplateId);
            if (!template?.permission_ids) throw new Error('Template not found');

            await get().updateUserPermissions(userId, template.permission_ids, grantedBy);
        } catch (error) {
            set({ error: error instanceof Error ? error.message : 'Failed to apply role template' });
        }
    },
    ...
    createRoleTemplate: async (roleTemplate: RoleTemplateFormData) => {
        try {
            ...
            if (roleTemplate.permission_ids.length > 0) {
                const templatePermissions = roleTemplate.permission_ids.map(permissionId => ({
            ...
            set({ error: error instanceof Error ? error.message : 'Failed to create role template' });
        }
    },
    ...
    setSortBy: (sortBy: 'name' | 'level' | 'resource' | 'created_at') => set({ sortBy }),
    setSortOrder: (order: 'asc' | 'desc') => set({ sortOrder: order }),
    setFilters: (filters: Partial<PermissionFilters>) => set({ filters: { ...get().filters, ...filters } }),
