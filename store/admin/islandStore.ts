...
interface IslandStoreState extends BaseStoreState<Island>, FilterableStoreState<Island, IslandFilters>, StatsStoreState<IslandStats> {
    // Computed data
...
interface IslandStoreActions extends BaseCrudActions<Island, IslandFormData>, SearchableActions<Island> {
...
const validateIslandFormData = (data: Partial<IslandFormData>): ValidationResult => {
...
export const useIslandStore = create<IslandStore>((set, get) => ({
    ...
    fetchById: async (id: string) => {
        ...
        try {
            ...
            set((state) => ({
                error: error instanceof Error ? error.message : 'Failed to fetch island',
                loading: { ...state.loading, singleIsland: false }
            }));
            return null;
        }
    ...
    create: async (data: IslandFormData) => {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to create island';
            set((state) => ({
                error: errorMessage,
                loading: { ...state.loading, creating: false }
            }));
    ...
    update: async (id: string, data: Partial<IslandFormData>) => {
        ...
        try {
            ...
            set((state) => ({
                data: state.data.map(island => island.id === id ? processedIsland : island),
                currentItem: state.currentItem?.id === id ? processedIsland : state.currentItem,
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to update island';
            set((state) => ({
                error: errorMessage,
                loading: { ...state.loading, updating: false }
            }));
    ...
    delete: async (id: string) => {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to delete island';
            set((state) => ({
                error: errorMessage,
                loading: { ...state.loading, deleting: false }
            }));
    ...
    getFilteredIslands: () => {
        ...
        if (filters.is_active !== undefined && filters.is_active !== null) {
            filtered = filtered.filter(island => island.is_active === filters.is_active);
        }
    ...
    calculateStats: () => {
        ...
        stats.recentlyUpdated = data.filter(island =>
            new Date(island.updated_at) > sevenDaysAgo
        ).length;
