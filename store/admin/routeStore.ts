...
interface RouteStoreState extends BaseStoreState<Route>, FilterableStoreState<Route, RouteFilters>, StatsStoreState<RouteStats> {
    // Computed data
...
interface RouteStoreActions extends BaseCrudActions<Route, RouteFormData>, SearchableActions<Route> {
...
const validateRouteFormData = (data: Partial<RouteFormData>): ValidationResult => {
...
export const useRouteStore = create<RouteStore>((set, get) => ({
    ...
    update: async (id: string, data: Partial<RouteFormData>) => {
        ...
        try {
            ...
            if (data.from_island_id !== undefined) updateData.from_island_id = data.from_island_id;
            if (data.to_island_id !== undefined) updateData.to_island_id = data.to_island_id;
            if (data.base_fare !== undefined) updateData.base_fare = data.base_fare;
            if (data.distance !== undefined) updateData.distance = data.distance?.trim();
            ...
            set((state) => ({
                data: state.data.map(route => route.id === id ? processedRoute : route),
                currentItem: state.currentItem?.id === id ? processedRoute : state.currentItem,
    ...
    delete: async (id: string) => {
        ...
        try {
            ...
            if (trips && trips.length > 0) {
                throw new Error('Cannot delete route with scheduled trips. Please cancel or reschedule all future trips first.');
            }
    ...
    setSortBy: (sortBy: 'name' | 'base_fare' | 'created_at' | 'total_trips_30d' | 'total_revenue_30d' | 'average_occupancy_30d') => {
        set({ sortBy });
    },
    ...
    getFilteredRoutes: () => {
        ...
        if (filters.from_island_id) {
            filtered = filtered.filter(route => route.from_island_id === filters.from_island_id);
        }

        if (filters.to_island_id) {
            filtered = filtered.filter(route => route.to_island_id === filters.to_island_id);
        }
    ...
    calculateStats: () => {
        ...
        const stats: RouteStats = {
            total: data.length,
            active: data.filter(route => route.status === 'active' || route.is_active).length,
            inactive: data.filter(route => route.status === 'inactive' || !route.is_active).length,
            totalTrips30d: 0,
            totalBookings30d: 0,
            totalRevenue30d: 0,
            avgOccupancy: 0,
        ...
        // Calculate averages
        stats.avgFare = data.length > 0 ? data.reduce((sum, route) => sum + route.base_fare, 0) / data.length : 0;
        stats.avgOccupancy = routesWithOccupancy > 0 ? totalOccupancy / routesWithOccupancy : 0;
        stats.onTimePerformance = routesWithOnTime > 0 ? totalOnTime / routesWithOnTime : 0;
        stats.cancellationRate = routesWithCancellation > 0 ? totalCancellation / routesWithCancellation : 0;
        stats.popularityScore = routesWithPopularity > 0 ? totalPopularity / routesWithPopularity : 0;
        ...
        if (data.length > 0) {
            ...
            stats.topRouteByRevenue = { route: topRevenue.name, revenue: topRevenue.total_revenue_30d };

            const topTrips = data.reduce((max, route) =>
                route.total_trips_30d > max.total_trips_30d ? route : max
            );
            stats.topRouteByTrips = { route: topTrips.name, trips: topTrips.total_trips_30d };

            const topOccupancy = data.reduce((max, route) =>
                route.average_occupancy_30d > max.average_occupancy_30d ? route : max
            );
            stats.topRouteByOccupancy = { route: topOccupancy.name, occupancy: topOccupancy.average_occupancy_30d };
        }
