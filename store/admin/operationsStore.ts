...
export const useOperationsStore = create<OperationsStore>((set, get) => ({
  ...
  fetchTodaySchedule: async () => {
    ...
    try {
      ...
      try {
        ...
        console.error(
          'Fallback fetchTodaySchedule also failed:',
          fallbackError
        );
  ...
  refreshAll: async () => {
    ...
    try {
      ...
      if (!statsError && enhancedStats) {
        ...
        console.warn(
          'OperationsStore: Enhanced stats not available:',
          statsError
        );
  ...
  calculateBasicStats: async () => {
    try {
      ...
      const totalCapacity = todayVessels.reduce(
