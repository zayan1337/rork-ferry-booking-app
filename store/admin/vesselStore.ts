...
export const useVesselStore = create<VesselStoreState & VesselStoreActions>(
  (set, get) => ({
    // ========================================================================
    // INITIAL STATE
    // ========================================================================
    ...
    fetchById: async (id: string) => {
      ...
      try {
        ...
        set({
          error:
            error instanceof Error ? error.message : 'Failed to fetch vessel',
          loading: { ...get().loading, fetchById: false },
        });
        return null;
      }
    ...
    fetchVesselDetails: async (id: string) => {
      ...
      try {
        ...
        const { data: layoutData, error: layoutError } = await supabase.rpc(
          'get_vessel_seat_layout',
          { vessel_id_param: id }
        );
      ...
      try {
        ...
        set({
          error:
            error instanceof Error
              ? error.message
              : 'Failed to fetch vessels by status',
          loading: { ...get().loading, fetchByStatus: false },
        });
        return [];
      }
        ...
        if (vessel && data.seating_capacity && data.seating_capacity > 0) {
          try {
            if (customSeatLayout) {
              ...
              await supabase.from('seats').delete().eq('vessel_id', vessel.id);
            }
          } catch (layoutError) {
    ...
    generateAutomaticSeatLayout: async (
      vesselId: string,
      capacity: number,
      vesselType: string
    ) => {
      set({
        loading: { ...get().loading, generateAutoLayout: true },
        error: null,
      });
      ...
      try {
        ...
        set({
          error:
            error instanceof Error
              ? error.message
              : 'Failed to generate automatic seat layout',
          loading: { ...get().loading, generateAutoLayout: false },
        });
        throw error;
      }
      ...
      try {
        ...
        if (vessel && customSeatLayout) {
          try {
            ...
            console.warn(
              'Failed to save custom seat layout during vessel update:',
              layoutError
            );
        ...
        if (
          ...
          try {
            ...
            if (!existingSeats) {
              ...
              await get().generateAutomaticSeatLayout(
                id,
                data.seating_capacity,
            ...
            console.warn(
              'Failed to generate seat layout during update:',
              layoutError
            );
        ...
        set({
          ...
          currentItem:
            get().currentItem?.id === id ? vessel : get().currentItem,
    ...
    fetchSeatLayout: async (vesselId: string) => {
      ...
      try {
        ...
        const { data, error } = await supabase.rpc('get_vessel_seat_layout', {
          vessel_id_param: vesselId,
        });
    ...
    updateSeats: async (vesselId: string, seats: Seat[]) => {
      ...
      try {
        ...
        await supabase.from('seats').delete().eq('vessel_id', vesselId);
    ...
    saveCustomSeatLayout: async (
      vesselId: string,
      layoutData: any,
      seats: Seat[]
    ) => {
      set({
        loading: { ...get().loading, saveCustomLayout: true },
        error: null,
      });
      ...
      try {
        ...
        const { error } = await supabase.rpc('save_vessel_seat_layout', {
          vessel_id_param: vesselId,
        ...
        set({
          error:
            error instanceof Error
              ? error.message
              : 'Failed to save custom seat layout',
          loading: { ...get().loading, saveCustomLayout: false },
        });
        throw error;
      }
    ...
    generateFerryLayout: async (
      ...
      try {
        ...
        if (
          layoutConfig &&
          layoutConfig.floors &&
          layoutConfig.floors[0] &&
          layoutConfig.floors[0].seat_count > 0
        ) {
          // This is a custom layout with predefined seat data

          // Extract seat data from the floors
          if (
            layoutConfig.floors[0].customSeats &&
            layoutConfig.floors[0].customSeats.length > 0
          ) {
          ...
          layoutData = {
            ...
            layout_data: {
              ...
              aisles:
                layoutConfig.aisles || layoutConfig.floors[0].aisles || [],
              rowAisles:
                layoutConfig.rowAisles ||
                layoutConfig.floors[0].rowAisles ||
                [],
              premium_rows:
                layoutConfig.premium_rows ||
                layoutConfig.floors[0].premium_rows ||
                [],
              disabled_seats:
                layoutConfig.disabled_seats ||
                layoutConfig.floors[0].disabled_seats ||
                [],
              crew_seats:
                layoutConfig.crew_seats ||
                layoutConfig.floors[0].crew_seats ||
                [],
          ...
          const ferryLayout = generateFerrySeatLayout(capacity, vesselType, {
            ...optimalConfig,
            ...layoutConfig,
          });
        ...
        const result = await get().saveCustomSeatLayout(
          vesselId,
        ...
        set({
          error:
            error instanceof Error
              ? error.message
              : 'Failed to generate ferry layout',
          loading: { ...get().loading, generateLayout: false },
        });
        throw error;
      }
    ...
    filterItems: (items, filters) => {
      ...
      if (filters.status && filters.status !== 'all') {
        filtered = filtered.filter(
          item => item && item.status === filters.status
        );
      }

      if (filters.is_active !== undefined && filters.is_active !== null) {
        filtered = filtered.filter(
          item => item && item.is_active === filters.is_active
        );
      }

      if (filters.min_capacity) {
        filtered = filtered.filter(
          item => item && item.seating_capacity >= filters.min_capacity!
        );
      }

      if (filters.max_capacity) {
        filtered = filtered.filter(
          item => item && item.seating_capacity <= filters.max_capacity!
        );
      }
    ...
    calculateComputedData: () => {
      ...
      const stats = {
        ...
        maintenance: validData.filter(v => v && v.status === 'maintenance')
          .length,
        totalTrips30d: validData.reduce(
          (sum, v) => sum + (v?.total_trips_30d || 0),
          0
        ),
        totalBookings30d: validData.reduce(
          (sum, v) => sum + (v?.total_bookings_30d || 0),
          0
        ),
        totalRevenue30d: validData.reduce(
        ...
        avgCapacity:
          validData.length > 0
            ...
            : 0,
        totalCapacity: validData.reduce(
          (sum, v) => sum + (v?.seating_capacity || 0),
          0
        ),
      };
