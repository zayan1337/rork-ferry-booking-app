...
interface ZoneStoreState extends
    BaseStoreState<Zone>,
...
interface ZoneStoreActions extends
...
const validateZoneFormData = (data: Partial<ZoneFormData>): ValidationResult => {
...
export const useZoneStore = create<ZoneStore>((set, get) => ({
    ...
    create: async (data: ZoneFormData) => {
        ...
        try {
            ...
            set((state) => ({
                data: [processedZone, ...state.data].sort((a, b) => a.order_index - b.order_index),
    ...
    update: async (id: string, data: Partial<ZoneFormData>) => {
        ...
        try {
            ...
            set((state) => ({
                data: state.data.map(zone => zone.id === id ? processedZone : zone)
                    .sort((a, b) => a.order_index - b.order_index),
                currentItem: state.currentItem?.id === id ? processedZone : state.currentItem,
    ...
    delete: async (id: string) => {
        ...
        try {
            ...
            if (islands && islands.length > 0) {
                throw new Error('Cannot delete zone that contains islands. Please move or delete all islands first.');
            }
    ...
    validateOrder: (orderIndex: number, excludeId?: string) => {
        ...
        if (orderIndex < 0) {
            return { isValid: false, errors: { order_index: 'Order index must be non-negative' } };
        }

        if (orderIndex > zones.length) {
            return { isValid: false, errors: { order_index: 'Order index is too high' } };
        }
    ...
    reorderZones: async (zoneOrders: { id: string; order_index: number }[]) => {
        ...
        try {
            ...
            for (const zoneOrder of zoneOrders) {
                const validation = get().validateOrder(zoneOrder.order_index, zoneOrder.id);
                if (!validation.isValid) {
                    throw new Error(`Invalid order for zone: ${Object.values(validation.errors)[0]}`);
                }
            }
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to reorder zones';
            set((state) => ({
                error: errorMessage,
                loading: { ...state.loading, reordering: false }
            }));
    ...
    calculateStats: () => {
        ...
        // Calculate averages
        if (data.length > 0) {
            stats.avgIslandsPerZone = Math.round((stats.totalIslands / data.length) * 100) / 100;
            stats.avgRoutesPerZone = Math.round((stats.totalRoutes / data.length) * 100) / 100;
        }
    ...
    getZoneByCode: (code: string) => {
        return get().data.find(zone => zone.code.toLowerCase() === code.toLowerCase());
    },
