...
interface AgentBookingsState {
    ...
    modifyBooking: (agentId: string, bookingId: string, modificationData: any) => Promise<{ bookingId: string; returnBookingId: string | null }>;
    agentCancelBooking: (agentId: string, bookingId: string, cancellationData: {
        reason: string;
    ...
    updateBookingStatusWithHistory: (agentId: string, bookingId: string, status: string, notes?: string) => Promise<void>;
    clearError: () => void;
    reset: () => void;
    ...
    refreshBookingsData: (agentId: string, clientsFetcher?: () => Promise<any[]>, statsFetcher?: () => Promise<any>) => Promise<void>;
    handleBookingCreated: (agentId: string, bookingId: string, returnBookingId?: string | null, clientsFetcher?: () => Promise<any[]>, statsFetcher?: () => Promise<any>) => Promise<void>;
...
export const useAgentBookingsStore = create<AgentBookingsState>((set, get) => ({
    ...
    modifyBooking: async (agentId: string, bookingId: string, modificationData: any) => {
        try {
            if (!agentId) throw new Error('Agent ID required');
            ...
            if (refreshError || !refreshedBooking?.booking_number) {
                console.error('Failed to fetch booking with booking_number:', refreshError);
            ...
            if (qrUpdateError) {
                console.error('Failed to update QR code URL for modified booking:', qrUpdateError);
            ...
            if (currentBooking.passengers && currentBooking.passengers.length > 0) {
                const passengerInserts = currentBooking.passengers.map((passenger: any) => ({
                    booking_id: newBooking.id,
                ...
                if (passengersError) {
                    console.error('Failed to copy passengers to modified booking:', passengersError);
                    // Don't throw error - booking was created successfully
                }
            }
            ...
            if (modificationData.selectedSeats && modificationData.selectedSeats.length > 0) {
                ...
                const seatReservations = modificationData.selectedSeats.map((seat: any) => ({
                    trip_id: modificationData.newTripId || currentBooking.trip_id,
                    seat_id: seat.id,
                    booking_id: newBooking.id,
                    is_available: false,
                ...
                if (seatError) {
                    console.error('Failed to create seat reservations for modified booking:', seatError);
                    // Don't throw error - booking was created successfully
                }
            }
            ...
            if (currentBooking.is_round_trip && modificationData.newReturnTripId) {
                ...
                if (returnBookingError) {
                    console.error('Failed to create modified return booking:', returnBookingError);
                } else {
                    ...
                    const { data: refreshedReturnBooking, error: returnRefreshError } = await supabase
                        .from('bookings')
                        .select('*')
                    ...
                    if (returnRefreshError || !refreshedReturnBooking?.booking_number) {
                        ...
                        if (returnQrUpdateError) {
                            console.error('Failed to update return QR code URL for modified booking:', returnQrUpdateError);
                        } else {
                            // Verify return QR code was saved
                            const { data: verifyReturnBooking, error: verifyReturnError } = await supabase
                                .from('bookings')
                                .select('qr_code_url')
                                .eq('id', returnBooking.id)
                                .single();
                            ...
                            if (verifyReturnError || !verifyReturnBooking?.qr_code_url) {
                                console.error('Return QR code verification failed:', verifyReturnError);
                            } else {
                                console.log('Return QR code successfully saved for modified booking:', returnBooking.id);
                            }
                        }
                    }
                    ...
                    if (currentBooking.passengers && currentBooking.passengers.length > 0) {
                        const returnPassengerInserts = currentBooking.passengers.map((passenger: any) => ({
                            booking_id: returnBooking.id,
                            seat_id: modificationData.returnSelectedSeats?.[currentBooking.passengers.indexOf(passenger)]?.id || passenger.seat_id,
                            passenger_name: passenger.passenger_name,
                            passenger_contact_number: passenger.passenger_contact_number,
                            special_assistance_request: passenger.special_assistance_request,
                        }));
                        ...
                        if (returnPassengersError) {
                            console.error('Failed to copy passengers to modified return booking:', returnPassengersError);
                        }
                    }

                    // Handle return seat reservations
                    if (modificationData.returnSelectedSeats && modificationData.returnSelectedSeats.length > 0) {
                        const returnSeatReservations = modificationData.returnSelectedSeats.map((seat: any) => ({
                            trip_id: modificationData.newReturnTripId,
                            seat_id: seat.id,
                            booking_id: returnBooking.id,
                            is_available: false,
                        ...
                        if (returnSeatError) {
                            console.error('Failed to create return seat reservations for modified booking:', returnSeatError);
                        }
                    }
                }
            }
    ...
    agentCancelBooking: async (agentId: string, bookingId: string, cancellationData: {
        reason: string;
        refundPercentage?: number;
        refundMethod?: 'agent_credit' | 'original_payment' | 'bank_transfer';
        bankDetails?: {
    ...
    updateBookingStatusWithHistory: async (agentId: string, bookingId: string, status: string, notes?: string) => {
        try {
            if (!agentId) throw new Error('Agent ID required');
    ...
    // Booking refresh utilities
    refreshBookingsData: async (agentId: string, clientsFetcher?: () => Promise<any[]>, statsFetcher?: () => Promise<any>) => {
        try {
            if (!agentId) return;
    ...
    handleBookingCreated: async (agentId: string, bookingId: string, returnBookingId?: string | null, clientsFetcher?: () => Promise<any[]>, statsFetcher?: () => Promise<any>) => {
