...
interface AgentState {
    ...
    modifyBooking: (bookingId: string, modificationData: any) => Promise<{ bookingId: string; returnBookingId: string | null }>;
        ...
        (set, get) => ({
            ...
            refreshAllData: async () => {
                ...
                try {
                    ...
                    set({
                        error: error instanceof Error ? error.message : 'Failed to refresh agent data',
                        isLoadingBookings: false,
                        isLoadingClients: false,
                        isLoadingStats: false,
                        isLoadingCredit: false,
            ...
            getAgentProfile: (agentId: string) => useAgentAuthStore.getState().getAgentProfile(agentId),
            getAgentClients: (agentId: string) => useAgentClientsStore.getState().getAgentClients(agentId),
            getAgentBookings: (agentId: string) => useAgentBookingsStore.getState().getAgentBookings(agentId),
            getAgentCreditTransactions: (agentId?: string) => {
                const { agent } = get();
                const targetAgentId = agentId || requireAgent(agent).id;
                return useAgentCreditStore.getState().getAgentCreditTransactions(targetAgentId);
            },
            getTranslations: (languageCode: string, context?: string) => 
                useAgentAuthStore.getState().getTranslations(languageCode, context),
            getUserLanguage: (userId: string) => useAgentAuthStore.getState().getUserLanguage(userId),
            getTextDirection: (languageCode: string) => useAgentAuthStore.getState().getTextDirection(languageCode),
            ...
            login: async (agentId: string, password: string) => {
                try {
                    ...
                    const success = await useAgentAuthStore.getState().login(agentId, password);
                    if (success) {
                        const { agent } = useAgentAuthStore.getState();
                        if (agent) {
                            set({
            ...
            fetchCreditTransactions: async () => {
                ...
                try {
                    ...
                    await useAgentCreditStore.getState().fetchCreditTransactions(agent.id);
                    get().syncFromSubStores();
            ...
            getBookingsByClient: (clientId: string) => {
                return useAgentBookingsStore.getState().getBookingsByClient(clientId, get().clients);
            },
            ...
            createBooking: async (bookingData: any) => {
                ...
                const bookingId = await useAgentBookingsStore.getState().createBooking(agent.id, bookingData);
            ...
            updateBookingStatus: async (bookingId: string, status: Booking['status']) => {
                await useAgentBookingsStore.getState().updateBookingStatus(bookingId, status);
                await get().refreshBookingsData();
            },
            ...
            modifyBooking: async (bookingId: string, modificationData: any) => {
                const agent = requireAgent(get().agent);
                const result = await useAgentBookingsStore.getState().modifyBooking(agent.id, bookingId, modificationData);
                await get().refreshBookingsData();
                return result;
            },
            ...
            agentCancelBooking: async (bookingId: string, cancellationData: any) => {
                const agent = requireAgent(get().agent);
                const cancellationId = await useAgentBookingsStore.getState().agentCancelBooking(agent.id, bookingId, cancellationData);
                await get().refreshBookingsData();
            ...
            addClientToAgent: async (clientId: string) => {
                const agent = requireAgent(get().agent);
                await useAgentClientsStore.getState().addClientToAgent(agent.id, clientId);
                get().syncFromSubStores();
            },
             ...
             */
            searchExistingUser: (email: string) => useAgentClientsStore.getState().searchExistingUser(email),
             ...
             */
            getTranslation: (key: string) => useAgentAuthStore.getState().getTranslation(key),
            ...
            refreshBookingsData: async () => {
                ...
                await bookingsStore.refreshBookingsData(agent.id, clientsFetcher, statsFetcher);
                get().syncFromSubStores();
            },
            ...
            handleBookingCreated: async (bookingId: string, returnBookingId?: string | null) => {
                ...
                await bookingsStore.handleBookingCreated(agent.id, bookingId, returnBookingId, clientsFetcher, statsFetcher);
                get().syncFromSubStores();
            },
             ...
             */
            getBookingModifications: (bookingId: string) => useAgentBookingsStore.getState().getBookingModifications(bookingId),
             ...
             */
            getBookingCancellation: (bookingId: string) => useAgentBookingsStore.getState().getBookingCancellation(bookingId),
             ...
             */
            getBookingFullHistory: (bookingId: string) => useAgentBookingsStore.getState().getBookingFullHistory(bookingId),
            ...
            updateBookingStatusWithHistory: async (bookingId: string, status: string, notes?: string) => {
                const agent = requireAgent(get().agent);
                await useAgentBookingsStore.getState().updateBookingStatusWithHistory(agent.id, bookingId, status, notes);
                await get().refreshBookingsData();
            },
