...
export const useZoneManagement = (
    ...
    async (data: ZoneFormData) => {
      try {
        ...
        const enhancedData = {
          ...
          order_index:
            data.order_index || getSuggestedZoneOrderIndex(zones || []),
        };
  ...
  const validateZoneOrder = useCallback(
  ...
  const moveZone = useCallback(
    async (zoneId: string, newOrderIndex: number) => {
      try {
  ...
  const reorderZones = useCallback(
    async (zoneOrders: { id: string; order_index: number }[]) => {
      try {
        // Validate all order indices
        for (const zoneOrder of zoneOrders) {
          const validation = validateZoneOrder(
  ...
  // Validation
  const validateZoneDataCallback = useCallback(
  ...
  const filterZonesByStatusCallback = useCallback(
    (zones: Zone[], isActive: boolean | null) => {
      return filterZonesByStatus(zones, isActive);
    },
    []
  );

  const sortZonesCallback = useCallback(
    (
      zones: Zone[],
      sortBy: 'name' | 'code' | 'order_index' | 'created_at',
      order: 'asc' | 'desc'
