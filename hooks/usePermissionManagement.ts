...
export const usePermissionManagement = () => {
  ...
  const permissionsByCategory = useMemo(() => {
    return PermissionUtils.groupPermissionsByCategory(
      filteredPermissions,
      filteredCategories
    );
  ...
  const createPermission = useCallback(
    async (data: PermissionFormData): Promise<Permission> => {
  ...
  const removePermission = useCallback(
    async (id: string): Promise<void> => {
      ...
      if (templatesUsingPermission.length > 0) {
        const templateNames = templatesUsingPermission
          .map(t => t.name)
          .join(', ');
        throw new Error(
          `Cannot delete permission. It's used in role templates: ${templateNames}`
        );
      }
      ...
      if (usersWithPermission.length > 0) {
        throw new Error(
          `Cannot delete permission. It's assigned to ${usersWithPermission.length} user(s).`
        );
      }
  ...
  const createCategory = useCallback(
    async (data: PermissionCategoryFormData): Promise<void> => {
  ...
  const createTemplate = useCallback(
    async (data: RoleTemplateFormData): Promise<void> => {
      ...
      if (permission?.dependencies) {
        ...
        if (!dependencyCheck.satisfied) {
          const missingPermissions = dependencyCheck.missing
            ...
            .join(', ');
          throw new Error(
            `Cannot grant permission. Missing dependencies: ${missingPermissions}`
          );
        }
      }
  ...
  const revokeUserPermission = useCallback(
    async (userId: string, permissionId: string): Promise<void> => {
      // Check if other permissions depend on this one
      const dependents = PermissionUtils.getPermissionDependents(
        permissionId,
        permissions
      );
      const userCurrentPermissions = getUserPermissions(userId);
      ...
      if (conflictingDependents.length > 0) {
        const dependentNames = conflictingDependents
          .map(p => p.name)
          .join(', ');
        throw new Error(
          `Cannot revoke permission. Other permissions depend on it: ${dependentNames}`
        );
      }
  ...
  const updateAllUserPermissions = useCallback(
    async (
      userId: string,
      ...
      for (const permissionId of permissionIds) {
        ...
        if (!dependencyCheck.satisfied) {
          ...
          const missingPermissions = dependencyCheck.missing
            ...
            .join(', ');
          throw new Error(
            `Cannot assign "${permission?.name}". Missing dependencies: ${missingPermissions}`
          );
        }
      }
  ...
  const bulkSelectPermissions = useCallback(
    (
      action: 'all' | 'none' | 'category' | 'level' | 'resource',
      value?: string
    ): string[] => {
      switch (action) {
        case 'all':
        ...
        case 'category':
          return value
            ? PermissionUtils.selectCategoryPermissions(
                value,
                filteredPermissions
              )
            : [];
        case 'level':
          return value
            ? PermissionUtils.selectPermissionsByLevel(
                value,
                filteredPermissions
              )
            : [];
        case 'resource':
          return value
            ? PermissionUtils.selectPermissionsByResource(
                value,
                filteredPermissions
              )
            : [];
  ...
  const getPermission = useCallback(
    (id: string): Permission | undefined => {
  ...
  const getCategory = useCallback(
    (id: string): PermissionCategory | undefined => {
  ...
  const getRoleTemplate = useCallback(
    (id: string): RoleTemplate | undefined => {
  ...
  const validatePermissionData = useCallback(
    (data: Partial<PermissionFormData>): ValidationResult => {
  ...
  const validateCategoryData = useCallback(
    (data: Partial<PermissionCategoryFormData>): ValidationResult => {
  ...
  const validateTemplateData = useCallback(
    (data: Partial<RoleTemplateFormData>): ValidationResult => {
  ...
  const applySorting = useCallback(
    (
      newSortBy: 'name' | 'level' | 'resource' | 'created_at',
...
export type PermissionManagementHook = ReturnType<
  typeof usePermissionManagement
>;
