...
export const useTripForm = ({
  ...
  const { trips, routes, vessels, addTrip, updateTrip, loading, setLoading } =
    useAdminStore();
  ...
  const validateForm = useCallback((): boolean => {
    ...
    const conflictCheck = validateTripConflicts(
      formData,
      trips,
      initialData?.id
    );
  ...
  const getFieldError = useCallback(
    (fieldName: keyof TripFormData): string | undefined => {
