...
export const useFormValidation = ({
  ...
  const setError = useCallback(
    (field: keyof BookingFormErrors, message: string) => {
  ...
  const validateBankDetailsForm = useCallback(
    (bankDetails: BankDetails): BookingFormErrors => {
  ...
  const validateCancellationForm = useCallback(
    (reason: string, bankDetails: BankDetails): boolean => {
      let isValid = true;
      const newErrors: BookingFormErrors = {};
