...
export const useBookingsData = () => {
  ...
  const filteredAndSortedBookings = useMemo(() => {
    let filtered = bookings.filter(booking => {
      ...
      const searchMatch =
        ...
        (booking?.routeName?.toLowerCase() || '').includes(
          filterState.searchQuery.toLowerCase()
        ) ||
        (booking?.id || '').includes(filterState.searchQuery) ||
        (booking?.customerEmail?.toLowerCase() || '').includes(
          filterState.searchQuery.toLowerCase()
        );
      ...
      const statusMatch =
        ...
        booking.status === filterState.filterStatus;
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      confirmedRate:
        bookings.length > 0
          ? ((confirmedCount / bookings.length) * 100).toFixed(1)
          : '0',
      reservedCount,
      cancelledCount,
      completedCount,
  ...
  const selectAllBookings = () => {
    setFilterState(prev => ({
      ...prev,
      selectedBookings:
        prev.selectedBookings.length === filteredAndSortedBookings.length
          ? []
