...
export const searchRoleTemplates = (
...
export const filterPermissionsByStatus = (
  permissions: Permission[],
...
export const filterPermissionsByCategory = (
  ...
  return permissions.filter(
    permission => permission.category_id === categoryId
  );
};
...
export const validatePermission = (
  ...
  if (!data.description?.trim()) {
    ...
    errors.description =
      'Permission description must be at least 10 characters';
  }
...
export const validatePermissionCategory = (
  data: Partial<PermissionCategory>
): ValidationResult => {
  const errors: Record<string, string> = {};
...
export const validateRoleTemplate = (
...
export const calculatePermissionStats = (permissions: Permission[]) => {
  ...
  // Group by level
  const byLevel = permissions.reduce(
    (acc, permission) => {
...
export const formatPermissionName = (
  resource: string,
  action: string
): string => {
...
export const checkPermissionDependencies = (
  ...
  const missing = permission.dependencies.filter(
    dep => !selectedPermissions.includes(dep)
  );
  return {
...
export const groupPermissionsByLevel = (
  permissions: Permission[]
): Record<string, Permission[]> => {
  return permissions.reduce(
    (acc, permission) => {
