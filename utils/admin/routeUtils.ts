...
export const validateRouteForm = (data: Partial<RouteFormData>): ValidationResult => {
    ...
    if (data.from_island_id && data.to_island_id && data.from_island_id === data.to_island_id) {
...
export const searchRoutes = (routes: Route[], query: string): Route[] => {
    ...
    return routes.filter(route =>
        ...
        (route.from_island_name && route.from_island_name.toLowerCase().includes(searchTerm)) ||
        (route.to_island_name && route.to_island_name.toLowerCase().includes(searchTerm)) ||
...
export const filterRoutes = (routes: Route[], filters: RouteFilters): Route[] => {
    return routes.filter(route => {
        ...
        if (filters.is_active !== undefined && route.is_active !== filters.is_active) {
            return false;
        }
        ...
        // Date filters
        if (filters.created_after && new Date(route.created_at) < new Date(filters.created_after)) {
            return false;
        }

        if (filters.created_before && new Date(route.created_at) > new Date(filters.created_before)) {
            return false;
        }
...
export const filterRoutesByStatus = (routes: Route[], status: string | null): Route[] => {
    if (!status || status === 'all') return routes;
    return routes.filter(route => route.status === status);
};

export const filterRoutesByIsland = (routes: Route[], islandId: string | null): Route[] => {
...
export const calculateRouteStats = (routes: Route[]): RouteStats => {
    ...
    const active = routes.filter(route => route.status === 'active' || route.is_active).length;
    ...
    routes.forEach(route => {
        ...
        if (route.on_time_performance_30d !== undefined && route.on_time_performance_30d > 0) {
    ...
    // Calculate averages
    const avgFare = total > 0 ? routes.reduce((sum, route) => sum + route.base_fare, 0) / total : 0;
    const avgOccupancy = routesWithOccupancy > 0 ? totalOccupancy / routesWithOccupancy : 0;
    const onTimePerformance = routesWithOnTime > 0 ? totalOnTime / routesWithOnTime : 0;
    const cancellationRate = routesWithCancellation > 0 ? totalCancellation / routesWithCancellation : 0;
    const popularityScore = routesWithPopularity > 0 ? totalPopularity / routesWithPopularity : 0;
    ...
    if (total > 0) {
        ...
        topRouteByRevenue = { route: topRevenue.name, revenue: topRevenue.total_revenue_30d };
        ...
        topRouteByOccupancy = { route: topOccupancy.name, occupancy: topOccupancy.average_occupancy_30d };
    }
...
export const getRoutePerformanceRating = (route: Route): 'excellent' | 'good' | 'fair' | 'poor' => {
    const onTimePerf = route.on_time_performance_30d || 0;
...
export const getRoutesByIsland = (routes: Route[], islandId: string): Route[] => {
    return routes.filter(route =>
        route.from_island_id === islandId || route.to_island_id === islandId
    );
};
...
export const formatDuration = (duration?: string): string => {
    ...
    return duration.includes('min') || duration.includes('hour') ? duration : `${duration} min`;
};
...
export const getRouteChanges = (original: Route, updated: Partial<RouteFormData>): Partial<RouteFormData> => {
    ...
    if (updated.from_island_id !== undefined && updated.from_island_id !== original.from_island_id) {
        changes.from_island_id = updated.from_island_id;
    }

    if (updated.to_island_id !== undefined && updated.to_island_id !== original.to_island_id) {
        changes.to_island_id = updated.to_island_id;
    }

    if (updated.base_fare !== undefined && updated.base_fare !== original.base_fare) {
        changes.base_fare = updated.base_fare;
    }

    if (updated.distance !== undefined && updated.distance !== original.distance) {
        changes.distance = updated.distance;
    }

    if (updated.duration !== undefined && updated.duration !== original.duration) {
...
export const isRouteNameUnique = (name: string, routes: Route[], currentRouteId?: string): boolean => {
...
export const isDuplicateRoute = (fromIslandId: string, toIslandId: string, routes: Route[], currentRouteId?: string): boolean => {
    return routes.some(route =>
...
export const getActiveRoutesForIsland = (routes: Route[], islandId: string): Route[] => {
    return routes.filter(route =>
        (route.from_island_id === islandId || route.to_island_id === islandId) &&
...
export const getRoutePopularityTrend = (routes: Route[]): 'up' | 'down' | 'stable' => {
...
export const getRouteCapacityUtilization = (routes: Route[]): number => {
    ...
    const routesWithGoodOccupancy = routes.filter(route => route.average_occupancy_30d >= 70).length;
