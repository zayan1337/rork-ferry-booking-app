...
export const validateIslandForm = (data: Partial<IslandFormData>): ValidationResult => {
...
export const filterIslands = (islands: Island[], filters: IslandFilters): Island[] => {
    return islands.filter(island => {
        // Active status filter
        if (filters.is_active !== undefined && island.is_active !== filters.is_active) {
            return false;
        }
        ...
        // Date filters
        if (filters.created_after && new Date(island.created_at) < new Date(filters.created_after)) {
            return false;
        }

        if (filters.created_before && new Date(island.created_at) > new Date(filters.created_before)) {
            return false;
        }
...
export const filterIslandsByStatus = (islands: Island[], isActive: boolean | null): Island[] => {
    if (isActive === null) return islands;
    return islands.filter(island => island.is_active === isActive);
};

export const filterIslandsByZone = (islands: Island[], zoneFilter: string | null): Island[] => {
    ...
    return [...islands].sort((a, b) => {
        ...
        switch (sortBy) {
            ...
            case 'zone':
                ...
                break;
            case 'zone_name':
                aValue = (a.zone_info?.name || a.zone_name || a.zone || '').toLowerCase();
                bValue = (b.zone_info?.name || b.zone_name || b.zone || '').toLowerCase();
                break;
            case 'created_at':
...
export const calculateIslandStats = (islands: Island[]): IslandStats => {
    ...
    islands.forEach(island => {
        const zoneName = island.zone_info?.name || island.zone_name || island.zone || 'Unknown Zone';
    ...
    const recentlyUpdated = islands.filter(island =>
        island.updated_at && new Date(island.updated_at) > sevenDaysAgo
    ).length;
...
export const getZoneStatistics = (islands: Island[], zones: any[]): Array<{
    zone: string;
    zoneId: string;
    totalIslands: number;
...
export const getIslandsByZone = (islands: Island[]): Record<string, Island[]> => {
    return islands.reduce((acc, island) => {
        const zoneKey = island.zone_info?.name || island.zone_name || island.zone || 'Unknown Zone';
        if (!acc[zoneKey]) {
            acc[zoneKey] = [];
        }
...
export const getIslandZoneDisplayName = (island: Island): string => {
    return island.zone_info?.name || island.zone_name || island.zone || 'Unknown Zone';
};
...
export const getIslandChanges = (original: Island, updated: Partial<IslandFormData>): Partial<IslandFormData> => {
