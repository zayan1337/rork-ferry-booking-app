...
export const filterVesselsByStatus = (vessels: Vessel[], status: string | null): Vessel[] => {
    ...
    if (filters.utilization_rating) {
        filtered = filtered.filter(vessel => {
            ...
            switch (filters.utilization_rating) {
                ...
                case 'good': return utilization >= 60 && utilization < 80;
                case 'fair': return utilization >= 40 && utilization < 60;
                case 'poor': return utilization < 40;
                default: return true;
            }
        });
    }
...
export const validateVesselData = (data: Partial<AdminManagement.VesselFormData>): AdminManagement.ValidationResult => {
...
export const getVesselById = (vessels: Vessel[], id: string): Vessel | undefined => {
    return vessels.find(vessel => vessel.id === id);
};

export const getVesselsByStatus = (vessels: Vessel[], status: string): Vessel[] => {
    return vessels.filter(vessel => vessel.status === status);
};

export const getTopVesselsByRevenue = (vessels: Vessel[], limit: number = 5): Vessel[] => {
    return vessels
        .sort((a, b) => (b.total_revenue_30d || 0) - (a.total_revenue_30d || 0))
        .slice(0, limit);
};

export const getTopVesselsByUtilization = (vessels: Vessel[], limit: number = 5): Vessel[] => {
    return vessels
        .sort((a, b) => (b.capacity_utilization_30d || 0) - (a.capacity_utilization_30d || 0))
        .slice(0, limit);
};
...
export const convertPositionToSeat = (position: SeatPosition, vesselId: string, layoutId?: string): Omit<Seat, 'id' | 'created_at' | 'updated_at'> => {
    return {
        vessel_id: vesselId,
        layout_id: layoutId,
...
export const validateSeatLayout = (layout: AdminManagement.SeatLayoutData): AdminManagement.ValidationResult => {
    const errors: Record<string, string> = {};
...
export const calculateOptimalLayout = (capacity: number, vesselType: string) => {
    let floors = 1;
...
export const validateFerryLayout = (layout: AdminManagement.SeatLayoutData): AdminManagement.ValidationResult => {
    const errors: Record<string, string> = {};
    ...
    layout.floors.forEach((floor, index) => {
        ...
        if (floor.aisles.some(aisle => aisle <= 0 || aisle > floor.columns)) {
            errors[`floor_${index}_aisles`] = 'Aisle positions must be valid column numbers';
        }

        if (floor.premium_rows.some(row => row <= 0 || row > floor.rows)) {
            errors[`floor_${index}_premium_rows`] = 'Premium row numbers must be valid row numbers';
        }
    });
