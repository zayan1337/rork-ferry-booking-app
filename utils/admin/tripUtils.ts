...
export const validateTripForm = (data: Partial<TripFormData>): ValidationResult => {
...
export const searchTrips = (trips: Trip[], query: string): Trip[] => {
    ...
    return trips.filter(trip =>
        ...
        (trip.delay_reason && trip.delay_reason.toLowerCase().includes(searchTerm)) ||
...
export const filterTrips = (trips: Trip[], filters: TripFilters): Trip[] => {
    return trips.filter(trip => {
        ...
        if (filters.departure_time_range) {
            ...
            if (tripTime < filters.departure_time_range.from || tripTime > filters.departure_time_range.to) {
                return false;
            }
        }
        ...
        if (filters.occupancy_range) {
            ...
            if (occupancy < filters.occupancy_range.min || occupancy > filters.occupancy_range.max) {
                return false;
            }
        }
        ...
        if (filters.fare_range && trip.base_fare) {
            ...
            if (totalFare < filters.fare_range.min || totalFare > filters.fare_range.max) {
                return false;
            }
        }
        ...
        // Date filters
        if (filters.created_after && new Date(trip.created_at) < new Date(filters.created_after)) {
            return false;
        }

        if (filters.created_before && new Date(trip.created_at) > new Date(filters.created_before)) {
            return false;
        }
...
export const filterTripsByStatus = (trips: Trip[], status: string | null): Trip[] => {
    if (!status || status === 'all') return trips;
    return trips.filter(trip => trip.status === status);
};

export const filterTripsByRoute = (trips: Trip[], routeId: string | null): Trip[] => {
    if (!routeId) return trips;
    return trips.filter(trip => trip.route_id === routeId);
};

export const filterTripsByVessel = (trips: Trip[], vesselId: string | null): Trip[] => {
    if (!vesselId) return trips;
    return trips.filter(trip => trip.vessel_id === vesselId);
};

export const filterTripsByDate = (trips: Trip[], date: string | null): Trip[] => {
...
export const calculateTripStats = (trips: Trip[]): TripStats => {
    ...
    const inProgress = trips.filter(trip => trip.status === 'boarding' || trip.status === 'departed').length;
    ...
    // Calculate averages
    const averageOccupancy = tripsWithOccupancy > 0 ? totalOccupancy / tripsWithOccupancy : 0;
    const onTimePerformance = total > 0 ? (onTimeTrips / total) * 100 : 0;
    const avgFare = total > 0 ? trips.reduce((sum, trip) => sum + ((trip.base_fare || 0) * trip.fare_multiplier), 0) / total : 0;
    ...
    if (total > 0) {
        const tripRevenues = trips.map(trip => ({
            ...
            revenue: (trip.base_fare || 0) * trip.fare_multiplier * (trip.confirmed_bookings || trip.booked_seats || 0)
        }));
        topTripByRevenue = tripRevenues.reduce((max, current) => 
            current.revenue > max.revenue ? current : max
        );

        const tripOccupancies = trips.filter(trip => trip.occupancy_rate).map(trip => ({
            trip: `${trip.route_name || 'Unknown Route'} - ${trip.travel_date} ${trip.departure_time}`,
    ...
    const topRouteByTrips = Object.keys(routeTripCounts).length > 0 ? 
        Object.entries(routeTripCounts).reduce((max, [route, trips]) => 
            trips > max.trips ? { route, trips } : max
        , { route: '', trips: 0 }) : undefined;
...
export const getTripPerformanceRating = (trip: Trip): 'excellent' | 'good' | 'fair' | 'poor' => {
    const occupancy = trip.occupancy_rate || 0;
...
export const formatTripName = (trip: Trip): string => {
    const routeName = trip.route_name || `${trip.from_island_name || 'Unknown'} → ${trip.to_island_name || 'Unknown'}`;
...
export const formatTripDuration = (trip: Trip): string => {
    if (trip.estimated_duration) {
        return trip.estimated_duration.includes('min') || trip.estimated_duration.includes('hour') 
...
export const getOccupancyLevel = (trip: Trip): 'low' | 'medium' | 'high' | 'full' => {
    const occupancy = trip.occupancy_rate || 0;
...
export const getStatusColor = (status: string): string => {
    switch (status) {
        ...
        case 'boarding': return '#F59E0B';  // yellow
        case 'departed': return '#10B981';  // green
        case 'arrived': return '#059669';   // dark green
...
export const getTripChanges = (original: Trip, updated: Partial<TripFormData>): Partial<TripFormData> => {
    const changes: Partial<TripFormData> = {};

    if (updated.route_id !== undefined && updated.route_id !== original.route_id) {
        changes.route_id = updated.route_id;
    }

    if (updated.vessel_id !== undefined && updated.vessel_id !== original.vessel_id) {
        changes.vessel_id = updated.vessel_id;
    }

    if (updated.travel_date !== undefined && updated.travel_date !== original.travel_date) {
        changes.travel_date = updated.travel_date;
    }

    if (updated.departure_time !== undefined && updated.departure_time !== original.departure_time) {
        changes.departure_time = updated.departure_time;
    }

    if (updated.arrival_time !== undefined && updated.arrival_time !== original.arrival_time) {
    ...
    if (updated.fare_multiplier !== undefined && updated.fare_multiplier !== original.fare_multiplier) {
...
export const getAvailableVesselsForDate = (trips: Trip[], allVessels: any[], date: string, time: string): any[] => {
    const busyVesselIds = trips
        .filter(trip => trip.travel_date === date && trip.departure_time === time && trip.is_active)
...
export const exportTripData = (trips: Trip[]) => {
    return trips.map(trip => ({
        ...
        fare_multiplier: trip.fare_multiplier,
        revenue: formatCurrency((trip.base_fare || 0) * trip.fare_multiplier * (trip.confirmed_bookings || trip.booked_seats || 0)),
...
export const getTripOccupancyTrend = (trips: Trip[]): 'up' | 'down' | 'stable' => {
...
export const generateTripsForSchedule = (request: TripGenerationRequest): GeneratedTrip[] => {
    const {
        route_id,
        vessel_id,
        start_date,
...
export const detectTripConflicts = (newTrips: GeneratedTrip[], existingTrips: Trip[]): {
    ...
    newTrips.forEach(newTrip => {
        ...
        const conflict = existingTrips.find(existing => 
...
export const getCommonTimeSlots = (): { label: string; value: string; category: string }[] => {
    return [
        // Morning slots
...
export const getDaysOfWeek = (): { label: string; value: number; short: string }[] => {
    return [
...
export const calculateTripCount = (request: Partial<TripGenerationRequest>): number => {
    if (!request.start_date || !request.end_date || !request.selected_days || !request.time_slots) {
        return 0;
    }
...
export const validateTripGenerationRequest = (request: Partial<TripGenerationRequest>): {
    isValid: boolean;
    ...
    if (request.fare_multiplier && (request.fare_multiplier < 0.1 || request.fare_multiplier > 5.0)) {
