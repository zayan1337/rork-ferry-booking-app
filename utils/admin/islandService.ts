 ...
 */
export const fetchIslandDetails = async (
  id: string
): Promise<IslandWithDetails | null> => {
  try {
...
export const updateIsland = async (
  id: string,
  updates: Partial<IslandFormData>
): Promise<Island> => {
  try {
...
export const deleteIsland = async (id: string): Promise<void> => {
  try {
    ...
    if (routes && routes.length > 0) {
      throw new Error(
        'Cannot delete island that has routes. Please delete the routes first.'
      );
    }
 ...
 * Bulk update islands
 */
export const bulkUpdateIslands = async (
  updates: { id: string; data: Partial<IslandFormData> }[]
): Promise<Island[]> => {
  try {
    const updatePromises = updates.map(update =>
      updateIsland(update.id, update.data)
    );
...
export const bulkDeleteIslands = async (ids: string[]): Promise<void> => {
  try {
    ...
    const { data: routes, error: routeCheckError } = await supabase
      ...
      .or(
        ids.map(id => `from_island_id.eq.${id},to_island_id.eq.${id}`).join(',')
      );
    ...
    if (routes && routes.length > 0) {
      ...
      if (conflictingIds.length > 0) {
        throw new Error(
          `Cannot delete islands that have routes. Islands with routes: ${conflictingIds.join(', ')}`
        );
      }
    }

    const { error } = await supabase.from('islands').delete().in('id', ids);
 ...
 */
export const checkIslandNameExists = async (
  name: string,
  excludeId?: string
): Promise<boolean> => {
  try {
 ...
 */
export const getIslandUsageStats = async (
  id: string
): Promise<{
  routeCount: number;
  ...
  try {
    ...
    if (routeCount > 0) {
      usageDetails.push(
        `${routeCount} route${routeCount > 1 ? 's' : ''} (${activeRouteCount} active)`
      );
    }
