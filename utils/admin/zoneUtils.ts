...
type OrderableUtilityFunctions<T> =
  AdminManagement.OrderableUtilityFunctions<T>;

// ============================================================================
// VALIDATION FUNCTIONS
// ============================================================================

export const validateZoneData = (
...
export const filterZonesByStatus = (
  zones: Zone[],
...
export const filterZonesByCode = (
  zones: Zone[],
...
export const getAvailableZoneOrderPositions = (
  zones: Zone[]
...
export const getNextZoneOrderIndex = (
  zones: Zone[],
  contextId?: string
): number => {
  return getSuggestedZoneOrderIndex(
    zones.filter(zone => zone.id !== contextId)
  );
};
...
export const calculateZoneStats = (zones: Zone[]): ZoneStats => {
  ...
  if (zones.length > 0) {
    stats.avgIslandsPerZone =
      Math.round((stats.totalIslands / zones.length) * 100) / 100;
    stats.avgRoutesPerZone =
      Math.round((stats.totalRoutes / zones.length) * 100) / 100;
  }
...
export const findZoneByCode = (
  zones: Zone[],
  code: string
): Zone | undefined => {
  return zones.find(zone => zone.code.toLowerCase() === code.toLowerCase());
};
...
export const getZonesByOrderRange = (
  zones: Zone[],
  minOrder: number,
  maxOrder: number
): Zone[] => {
  return zones.filter(
    zone => zone.order_index >= minOrder && zone.order_index <= maxOrder
  );
};
...
export const hasZoneChanged = (
  original: Zone,
  updated: Partial<ZoneFormData>
): boolean => {
  return (
    ...
    (updated.is_active !== undefined &&
      original.is_active !== updated.is_active) ||
    (updated.order_index !== undefined &&
      original.order_index !== updated.order_index)
  );
};
...
export const getZoneSelectOptions = (
  zones: Zone[],
  includeInactive: boolean = false
) => {
  const filteredZones = includeInactive
    ? zones
