...
-- Create role_permissions table for role-based permission assignments
...
-- Add super_admin flag to user_profiles table
ALTER TABLE public.user_profiles 
ADD COLUMN is_super_admin boolean NOT NULL DEFAULT false;

-- Create indices for performance
CREATE INDEX idx_user_permissions_user_id ON public.user_permissions USING btree (user_id) TABLESPACE pg_default;
CREATE INDEX idx_user_permissions_permission_id ON public.user_permissions USING btree (permission_id) TABLESPACE pg_default;
CREATE INDEX idx_role_permissions_role ON public.role_permissions USING btree (role) TABLESPACE pg_default;
CREATE INDEX idx_user_profiles_super_admin ON public.user_profiles USING btree (is_super_admin) WHERE is_super_admin = true;
...
-- Super Admin gets all permissions (will be set manually in user_profiles)
...
-- =============================================================================
-- CREATE ACTIVITY LOG TRIGGERS FOR PERMISSION CHANGES
-- =============================================================================
...
-- =============================================================================
-- USEFUL QUERIES FOR MANUAL SETUP
-- =============================================================================

-- Query to set a user as super admin (replace with actual user ID)
-- UPDATE user_profiles SET is_super_admin = true WHERE email = 'admin@example.com';

-- Query to view all permissions for a specific user
-- SELECT * FROM user_permissions_view WHERE email = 'admin@example.com';

-- Query to grant individual permission to a user
-- INSERT INTO user_permissions (user_id, permission_id, granted_by)
-- VALUES (
--   (SELECT id FROM user_profiles WHERE email = 'user@example.com'),
--   (SELECT id FROM permissions WHERE name = 'users.delete'),
--   (SELECT id FROM user_profiles WHERE email = 'superadmin@example.com')
-- ); 