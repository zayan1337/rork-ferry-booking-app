...
export const validateZoneForm = (data: ZoneFormData): ValidationError[] => {
  ...
  if (!data.code?.trim()) {
    ...
    errors.push({
      field: 'code',
      message: 'Zone code must be at least 2 characters',
    });
  } else if (data.code.trim().length > 10) {
    errors.push({
      field: 'code',
      message: 'Zone code must be less than 10 characters',
    });
...
export const calculateZoneStats = (zones: Zone[]) => {
  ...
  const withIslands = zones.filter(
    zone => (zone.total_islands || 0) > 0
  ).length;
  ...
  return {
    ...
    withIslandsPercentage:
      total > 0 ? Math.round((withIslands / total) * 100) : 0,
  };
};
...
export const compareZones = (original: Zone, updated: ZoneFormData) => {
  ...
  if (original.order_index !== updated.order_index) {
    changes.order_index = {
      from: original.order_index,
      to: updated.order_index,
    };
  }
...
export const isDuplicateZoneName = (
  zones: Zone[],
  name: string,
  excludeId?: string
): boolean => {
  return zones.some(
    zone =>
      zone.id !== excludeId && zone.name.toLowerCase() === name.toLowerCase()
  );
};

export const isDuplicateZoneCode = (
  zones: Zone[],
  code: string,
  excludeId?: string
): boolean => {
  return zones.some(
    zone =>
      zone.id !== excludeId && zone.code.toLowerCase() === code.toLowerCase()
  );
};
...
export const validateZoneData = (
  data: Partial<{ name: string; code: string; order_index: number }>
): {
  isValid: boolean;
