...
export const filterTermsByStatus = (
  terms: TermsAndConditions[],
  isActive: boolean
): TermsAndConditions[] => {
...
export const filterTermsByVersion = (
  terms: TermsAndConditions[],
  version: string
): TermsAndConditions[] => {
...
export const filterPromotionsByStatus = (
  promotions: Promotion[],
  isActive: boolean
): Promotion[] => {
...
export const filterPromotionsByPeriod = (
  promotions: Promotion[],
  period: 'current' | 'upcoming' | 'expired'
): Promotion[] => {
  const now = new Date();
...
export const calculatePromotionsStats = (promotions: Promotion[]) => {
    ...
    activePromotions.length > 0
      ? Math.round(
          activePromotions.reduce(
            (sum, promo) => sum + promo.discount_percentage,
            0
  ...
  return {
    ...
    firstTimeOnly: promotions.filter(promo => promo.is_first_time_booking_only)
      .length,
  };
};
...
export const getUniqueTermsVersions = (
  terms: TermsAndConditions[]
): string[] => {
  return [...new Set(terms.map(term => term.version))].sort();
};

export const getTermsByVersion = (
  terms: TermsAndConditions[],
  version: string
): TermsAndConditions[] => {
  return terms.filter(term => term.version === version);
...
export const validateTermsUniqueness = (
  terms: TermsAndConditions[],
  newTitle: string,
  newVersion: string,
  excludeId?: string
...
export const validatePromotionUniqueness = (
  promotions: Promotion[],
  newName: string,
  excludeId?: string
): boolean => {
