 ...
 */
export const generateRouteDisplayName = (
  fromIsland: Island,
  toIsland: Island
): string => {
...
export const filterRoutes = (
  ...
  return routes.filter(route => {
    ...
    if (filters.fare_range) {
      if (
        route.base_fare < filters.fare_range.min ||
        route.base_fare > filters.fare_range.max
      ) {
        return false;
      }
    }
    ...
    if (filters.distance_range) {
      ...
      if (!isNaN(distance)) {
        if (
          distance < filters.distance_range.min ||
          distance > filters.distance_range.max
        ) {
          return false;
        }
      }
 ...
 */
export const getRoutePopularityLevel = (
  route: Route
): 'low' | 'medium' | 'high' => {
    ...
    for (const toIsland of islands) {
      if (fromIsland.id !== toIsland.id) {
        const routeExists = existingRoutes.some(
          route =>
            route.from_island_id === fromIsland.id &&
            route.to_island_id === toIsland.id
        );
