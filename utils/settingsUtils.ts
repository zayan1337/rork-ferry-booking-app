...
import { PermissionLevel, AdminUser, SettingsStats, PermissionCategory, RoleTemplate, Permission } from "@/types/settings";
...
export const calculateSettingsStats = (
    ...
    const availablePermissions = permissionCategories.flatMap(cat => cat.permissions);
...
export const validateRoleName = (name: string, existingRoles: RoleTemplate[]): string | null => {
    if (!name.trim()) {
...
export const validatePermissionSelection = (permissions: string[]): string | null => {
...
export const getUserRoleColor = (roleId: string, roleTemplates: RoleTemplate[]): string => {
...
export const getPermissionsByCategory = (
    ...
    permissionCategories.forEach(category => {
        result[category.id] = category.permissions.filter(p => permissions.includes(p.id));
    });
...
export const formatSystemMetric = (value: number, type: 'percentage' | 'bytes' | 'count'): string => {
    switch (type) {
        case 'percentage':
