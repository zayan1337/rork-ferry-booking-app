...
export const getZoneActivityLogs = async (
  zoneId: string
): Promise<ZoneActivityLog[]> => {
  try {
    const { data, error } = await supabase
...
export const reorderZones = async (
  zoneUpdates: { id: string; order_index: number }[]
): Promise<void> => {
  try {
...
// Bulk update zones
export const bulkUpdateZones = async (
  zoneUpdates: { id: string; updates: Partial<ZoneFormData> }[]
): Promise<void> => {
  try {
...
export const importZonesFromCSV = async (
  csvData: string
    ...
    for (let i = 0; i < dataLines.length; i++) {
      try {
        ...
        const zoneData: ZoneFormData = {
          ...
          is_active:
            (
              values[headers.indexOf('is_active')] ||
              values[headers.indexOf('Active')] ||
              'true'
            ).toLowerCase() === 'true',
          order_index:
            parseInt(
              values[headers.indexOf('order_index')] ||
                values[headers.indexOf('Order')] ||
                '0'
            ) || 0,
        };
...
export const checkZoneExists = async (
  ...
  try {
    let query = supabase.from('zones').select('id, name, code');
    ...
    const nameExists =
      data?.some(zone => zone.name.toLowerCase() === name.toLowerCase()) ||
      false;
    const codeExists =
      data?.some(zone => zone.code.toLowerCase() === code.toLowerCase()) ||
      false;
