...
export const validateUserForm = (
  ...
  if (formData.address) {
    ...
    if (
      ...
      formData.address.street.trim().length > 200
    ) {
    ...
    if (
      ...
      formData.address.postal_code.trim().length > 20
    ) {
  ...
  if (formData.emergency_contact) {
    ...
    if (
      ...
      formData.emergency_contact.name.trim().length < 2
    ) {
    ...
    if (
      ...
      formData.emergency_contact.phone.trim().length === 0
    ) {
...
export const getUserActivityLevel = (
  user: UserProfile
): 'very_active' | 'active' | 'moderate' | 'inactive' => {
...
export const formatActivityLevel = (
  level: number | 'very_active' | 'active' | 'moderate' | 'inactive'
): { label: string; color: string } => {
 ...
 */
export const formatEngagementScore = (
  score: number
...
export const filterUsers = (
  ...
  return users.filter(user => {
    ...
    if (
      ...
      user.email_verified !== filters.email_verified
    ) {
      return false;
    }
    ...
    if (
      filters.mobile_verified !== undefined &&
      user.mobile_verified !== filters.mobile_verified
    ) {
      return false;
    }
    ...
    if (filters.location && user.address) {
      if (
        ...
        user.address.country !== filters.location.country
      ) {
        return false;
      }
      if (
        filters.location.city &&
        user.address.city !== filters.location.city
      ) {
        return false;
      }
    }
...
export const validateUserUniqueness = (
  ...
  const emailExists = existingUsers.some(
    user => user.email === formData.email && user.id !== currentUserId
  );
...
export const generateUserStatistics = (users: UserProfile[]) => {
  ...
  const verifiedUsers = users.filter(
    u => u.email_verified || u.mobile_verified
  ).length;
  const usersWithBookings = users.filter(
    u => (u.total_bookings || 0) > 0
  ).length;
  ...
  const averageAge =
    usersWithAge.length > 0
      ? usersWithAge.reduce((sum, user) => sum + getUserAge(user), 0) /
        usersWithAge.length
      : 0;
  ...
  return {
    ...
    active: activeUsers,
    activePercentage:
      total > 0 ? ((activeUsers / total) * 100).toFixed(1) : '0',
    verified: verifiedUsers,
    verifiedPercentage:
      total > 0 ? ((verifiedUsers / total) * 100).toFixed(1) : '0',
...
export const formatUserActivity = (
  ...
  const mapped = activityMap[
    activity.activity_type as keyof typeof activityMap
  ] || {
