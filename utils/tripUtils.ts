import { Trip, TripFormData, TripValidationErrors, TripFilters, Route, Vessel } from "@/types/operations";

/**
 * Validates trip form data
 */
export const validateTripForm = (formData: TripFormData): TripValidationErrors => {
    ...
    if (!formData.travel_date || formData.travel_date.trim().length === 0) {
        ...
        if (travelDate > maxDate) {
            errors.travel_date = "Travel date cannot be more than 1 year in the future";
        }
    }
 ...
 */
export const calculateEstimatedArrivalTime = (departureTime: string, routeDuration: string): string => {
 ...
 */
export const formatTripStatus = (status: string): { label: string; color: string } => {
    const statusMap = {
 ...
 */
export const getTripOccupancyLevel = (trip: Trip): "low" | "medium" | "high" | "full" => {
...
export const getTripPerformanceMetrics = (trip: Trip, route: Route) => {
    ...
    return {
        ...
        revenue: revenue,
        isOnTime: isOnTime,
        profitability: occupancy >= 60 ? "profitable" : occupancy >= 40 ? "marginal" : "unprofitable",
...
export const validateTripConflicts = (
    ...
    const conflictingTrip = existingTrips.find(trip =>
...
export const suggestDepartureTimes = (
    ...
    for (const time of standardTimes) {
        ...
        const hasConflict = busyTimes.some(busyTime =>
            Math.abs(minutes - busyTime) < 60 // 1 hour buffer
        );
 ...
 */
export const calculateTripTurnaroundTime = (trip: Trip, route: Route): number => {
 ...
 */
export const getWeatherImpact = (weatherConditions?: string): {
    ...
    if (conditions.includes("storm") || conditions.includes("heavy rain") || conditions.includes("rough seas")) {
        return { impact: "high", recommendation: "Consider cancelling or postponing trip" };
    }

    if (conditions.includes("rain") || conditions.includes("wind") || conditions.includes("choppy")) {
        return { impact: "medium", recommendation: "Monitor conditions closely and inform passengers" };
    }

    if (conditions.includes("cloudy") || conditions.includes("light wind")) {
        return { impact: "low", recommendation: "Normal operations with extra caution" };
    }
 ...
 */
export const formatTripDisplay = (trip: Trip): {
    displayName: string;
    timeRange: string;
