...
export const calculateIslandStats = (islands: DatabaseIsland[]) => {
  ...
  const zoneWithMostIslands = Object.entries(zoneGroups).reduce(
    (max, [zoneName, zoneIslands]) =>
      zoneIslands.length > max.count
        ? { zone: zoneName, count: zoneIslands.length }
        : max,
    { zone: '', count: 0 }
  );
  ...
  const recentlyAdded = islands.filter(
    island => new Date(island.created_at) >= thirtyDaysAgo
  ).length;
...
export const getZoneStatistics = (islands: DatabaseIsland[], zones: Zone[]) => {
  const zoneStats = zones.map(zone => {
    ...
    return {
      ...
      activationRate:
        zoneIslands.length > 0
          ? Math.round((activeIslands / zoneIslands.length) * 100)
          : 0,
    };
  });
 ...
 */
export const validateIslandData = (
  data: Partial<DatabaseIsland>
 ...
 */
export const isIslandNameUnique = (
  name: string,
  islands: DatabaseIsland[],
 ...
 */
export const getAvailableIslandsForRoutes = (
  islands: DatabaseIsland[]
): DatabaseIsland[] => {
 ...
 */
export const findIslandsByZoneId = (
  islands: DatabaseIsland[],
  zoneId: string
): DatabaseIsland[] => {
  return islands.filter(island => island.zone_id === zoneId);
 ...
 */
export const getIslandStatsForZone = (
  islands: DatabaseIsland[],
  zoneId: string
) => {
  ...
  return {
    ...
    activationRate:
      zoneIslands.length > 0
        ? Math.round((activeIslands.length / zoneIslands.length) * 100)
        : 0,
  };
};
