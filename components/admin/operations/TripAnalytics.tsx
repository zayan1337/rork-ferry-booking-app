...
export default function TripAnalytics({
    ...
    const [selectedMetric, setSelectedMetric] = useState<'revenue' | 'occupancy' | 'performance'>('revenue');

    // Calculate trip-specific metrics
    const totalCapacity = (trip.available_seats || 0) + (trip.booked_seats || 0);
    const occupancyRate = totalCapacity > 0 ? (trip.booked_seats / totalCapacity) * 100 : 0;
    const revenue = route ? (trip.booked_seats || 0) * route.base_fare * (trip.fare_multiplier || 1) : 0;
    ...
    const occupancyComparison = historicalData ? occupancyRate - historicalData.averageOccupancy : 0;
    const revenueComparison = historicalData ? revenue - historicalData.averageRevenue : 0;
    ...
    const performanceStats = [
        ...
        },
        {
            ...
            trend: occupancyComparison > 0 ? 'up' : occupancyComparison < 0 ? 'down' : 'neutral',
            color: isHighDemand ? colors.success : occupancyRate > 50 ? colors.warning : colors.error,
        },
        {
            title: 'Revenue/Seat',
        ...
        },
        {
            ...
            subtitle: historicalData ?
                ...
                'Status',
            icon: isOnTime ? <CheckCircle size={24} color={colors.success} /> : <AlertTriangle size={24} color={colors.warning} />,
            trend: 'neutral',
    ...
    const insightCards = [
        {
            title: 'Demand Level',
            value: isFullyBooked ? 'Sold Out' : isHighDemand ? 'High Demand' : occupancyRate > 50 ? 'Moderate' : 'Low',
            ...
            icon: isFullyBooked ? <Zap size={20} color={colors.warning} /> :
                isHighDemand ? <TrendingUp size={20} color={colors.success} /> :
                    <Activity size={20} color={colors.primary} />,
            color: isFullyBooked ? colors.warning : isHighDemand ? colors.success : colors.primary,
        },
        {
            ...
            icon: isProfitable ? <ThumbsUp size={20} color={colors.success} /> : <ThumbsDown size={20} color={colors.error} />,
            color: isProfitable ? colors.success : colors.error,
        },
        {
            title: 'Route Performance',
            value: route?.origin && route?.destination ? `${route.origin} ↔ ${route.destination}` : 'Route Analysis',
            ...
            <View style={styles.chartContainer}>
                <Text style={styles.chartTitle}>
                    {selectedMetric === 'revenue' ? 'Revenue Trend' :
                        selectedMetric === 'occupancy' ? 'Occupancy Trend' :
                            'Performance Trend'}
                </Text>
                ...
                <View style={styles.section}>
                    ...
                    <View style={styles.comparisonContainer}>
                        <View style={styles.comparisonItem}>
                            ...
                            <Text style={styles.comparisonValue}>{historicalData.averageOccupancy.toFixed(1)}%</Text>
                            <Text style={[
                                styles.comparisonDiff,
                                { color: occupancyComparison >= 0 ? colors.success : colors.error }
                            ]}>
                                {occupancyComparison >= 0 ? '+' : ''}{occupancyComparison.toFixed(1)}%
                            </Text>
                        </View>

                        <View style={styles.comparisonItem}>
                            <Text style={styles.comparisonLabel}>Average Revenue</Text>
                            <Text style={styles.comparisonValue}>{formatCurrency(historicalData.averageRevenue, 'MVR')}</Text>
                            <Text style={[
                                styles.comparisonDiff,
                                { color: revenueComparison >= 0 ? colors.success : colors.error }
                            ]}>
                                {revenueComparison >= 0 ? '+' : ''}{formatCurrency(revenueComparison, 'MVR')}
                            </Text>
                        </View>

                        <View style={styles.comparisonItem}>
                            <Text style={styles.comparisonLabel}>On-Time Rate</Text>
                            <Text style={styles.comparisonValue}>{historicalData.onTimePerformance.toFixed(1)}%</Text>
            ...
            <View style={styles.section}>
                ...
                <View style={styles.summaryContainer}>
                    ...
                    <View style={styles.summaryRow}>
                        ...
                        <Text style={styles.summaryValue}>{formatCurrency(revenuePerSeat, 'MVR')}</Text>
                    </View>
                    <View style={styles.summaryRow}>
                        <Text style={styles.summaryLabel}>Fare Multiplier Applied</Text>
                        <Text style={styles.summaryValue}>{trip.fare_multiplier || 1}x</Text>
                    </View>
                    {historicalData && (
                        <>
                            <View style={styles.summaryRow}>
                                <Text style={styles.summaryLabel}>Customer Satisfaction</Text>
                                <Text style={styles.summaryValue}>{(historicalData.customerSatisfaction * 100).toFixed(1)}%</Text>
                            </View>
                            <View style={styles.summaryRow}>
                                <Text style={styles.summaryLabel}>Cancellation Rate</Text>
                                <Text style={styles.summaryValue}>{(historicalData.cancellationRate * 100).toFixed(1)}%</Text>
                            </View>
                        </>
                    )}
                </View>
