...
export interface TripFiltersState {
    searchTerm: string;
    status: "all" | "scheduled" | "in-progress" | "completed" | "cancelled" | "delayed";
...
export interface TripSortConfig {
    field: "travel_date" | "departure_time" | "status" | "booked_seats" | "available_seats" | "fare_multiplier";
    direction: "asc" | "desc";
}
    ...
    const renderSortButton = (field: TripSortConfig['field'], label: string) => {
        ...
        return (
            ...
            >
                ...
                {isActive ? (
                    isAsc ? <SortAsc size={14} color={colors.primary} /> :
    ...
    return (
        <View style={styles.container}>
            ...
            <SearchFilterBar
                ...
                filterActions={[
                    {
                        icon: <Filter size={16} color={showAdvanced ? colors.white : colors.primary} />,
                        onPress: onToggleAdvanced || (() => { }),
                        variant: showAdvanced ? "primary" : "outline",
                        size: "medium",
                    },
                ]}
                rightActions={canManageTrips ? [
                    {
            ...
            >
                ...
                {renderFilterButton("In Progress", filters.status === "in-progress",
            ...
            {showAdvanced && (
                <View style={styles.advancedFilters}>
                    ...
                    <View style={styles.filterSection}>
                        ...
                        >
                            {renderFilterButton("All Time", filters.dateFilter === "all",
                                () => handleFilterChange("dateFilter", "all"))}
                            {renderFilterButton("Today", filters.dateFilter === "today",
                                () => handleFilterChange("dateFilter", "today"))}
                            {renderFilterButton("Tomorrow", filters.dateFilter === "tomorrow",
                                () => handleFilterChange("dateFilter", "tomorrow"))}
                            {renderFilterButton("This Week", filters.dateFilter === "thisWeek",
                                () => handleFilterChange("dateFilter", "thisWeek"))}
                            {renderFilterButton("This Month", filters.dateFilter === "thisMonth",
                    ...
                    <View style={styles.filterSection}>
                        ...
                        <ScrollView
                            ...
                            contentContainerStyle={styles.filterRow}
                        >
                            {renderFilterButton("All", filters.occupancyFilter === "all",
                                () => handleFilterChange("occupancyFilter", "all"))}
                            {renderFilterButton("Low (0-50%)", filters.occupancyFilter === "low",
                                () => handleFilterChange("occupancyFilter", "low"))}
                            {renderFilterButton("Medium (51-80%)", filters.occupancyFilter === "medium",
                                () => handleFilterChange("occupancyFilter", "medium"))}
                            {renderFilterButton("High (81-99%)", filters.occupancyFilter === "high",
                                () => handleFilterChange("occupancyFilter", "high"))}
                            {renderFilterButton("Full (100%)", filters.occupancyFilter === "full",
