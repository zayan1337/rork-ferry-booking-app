...
export default function VesselDetails({
  ...
  useEffect(() => {
    ...
    }, 800); // Short loading time for better UX
  ...
  const generateSeatLayoutFromDatabase = (
    seats: Seat[],
      ...
      return (
        <View style={styles.noSeatsContainer}>
          ...
          <Text style={styles.noSeatsMessage}>
            This vessel doesn't have any seats configured yet. Please set up the
            seat layout in the seat arrangement manager.
          </Text>
        </View>
      );
    }
    ...
    for (let rowIndex = 0; rowIndex < layoutConfig.rows; rowIndex++) {
      ...
      for (let colIndex = 0; colIndex < layoutConfig.columns; colIndex++) {
        ...
        if (
          layoutConfig.aisles.includes(colIndex + 1) &&
          colIndex < layoutConfig.columns - 1
        ) {
          rowSeats.push(
            <View
      ...
      rows.push(
        <View key={`row-${rowIndex + 1}`} style={styles.seatRow}>
          <View style={styles.seatRowContent}>{rowSeats}</View>
        </View>
      );
    }
      ...
      .forEach(rowNumber => {
        ...
        rows.push(
          <View key={`row-${rowNumber}`} style={styles.seatRow}>
            ...
            <View style={styles.seatRowContent}>{rowElements}</View>
          </View>
        );
      });
      ...
      {maintenanceStatus.status !== 'up_to_date' && (
        <View
          style={[
            ...
            { borderLeftColor: maintenanceStatus.color },
          ]}
        >
      ...
      <View style={styles.section}>
        <View style={styles.seatLayoutContainer}>
          <View style={styles.seatLayoutHeader}>
            <View style={styles.seatLayoutHeaderLeft}>
        ...
        <View style={styles.systemInfo}>
          ...
          {vessel.description && (
            <View style={styles.systemRow}>
              ...
              <Text style={styles.systemValue}>{vessel.description}</Text>
            </View>
          )}
        </View>
      </View>
      ...
      {vessel.status === 'maintenance' && (
        <View style={styles.warningContainer}>
          ...
          <Text style={styles.warningText}>
            This vessel is currently under maintenance and unavailable for
            service.
          </Text>
        </View>
      )}
