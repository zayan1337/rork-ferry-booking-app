...
const ContentTab: React.FC<ContentTabProps> = ({ isActive, searchQuery = "" }) => {
    ...
    const hasManagePermission = useMemo(() => canManageContent(), [canManageContent]);
    ...
    const handleAddContent = useCallback(() => {
        if (!hasManagePermission) {
            Alert.alert("Access Denied", `You don't have permission to create ${activeTab}.`);
            return;
        }
    ...
    const handleItemPress = useCallback((itemId: string) => {
        ...
        const route = activeTab === 'terms' ? `../terms/${itemId}` : `../promotion/${itemId}`;
        router.push(route as any);
    ...
    const handleEdit = useCallback((itemId: string) => {
        ...
        const route = activeTab === 'terms' ? `../terms/edit/${itemId}` : `../promotion/edit/${itemId}`;
        router.push(route as any);
    }, [activeTab, hasManagePermission]);

    const handleDelete = useCallback(async (itemId: string) => {
        if (!hasManagePermission) {
            Alert.alert("Access Denied", `You don't have permission to delete ${activeTab}.`);
            return;
        }

        const itemType = activeTab === 'terms' ? 'terms and conditions' : 'promotion';
        Alert.alert(
            `Delete ${itemType}`,
        ...
        <View style={styles.sectionContent}>
                    ...
                    <View style={styles.sectionTitleContainer}>
                        ...
                        <View>
                            ...
                            <Text style={styles.sectionSubtitle}>
                                {currentData.length} {activeTab === 'terms' ? 'terms & conditions' : 'promotions'} available
                            </Text>
                        </View>
                    </View>
                </View>
            ...
            <View style={styles.tabToggleContainer}>
                ...
                >
                    <Percent
                        size={16}
                        color={activeTab === 'promotions' ? colors.primary : colors.textSecondary}
                    />
                    <Text style={[
                        styles.tabToggleText,
    ...
    const renderItem = ({ item, index }: { item: TermsAndConditions | Promotion; index: number }) => {
        if (activeTab === 'terms') {
    ...
    const renderEmptyState = () => (
        <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyStateText}>
                {searchQuery ? 'Try adjusting your search terms' : `No ${activeTab} available`}
            </Text>
        </View>
    );
        ...
        return (
            <View style={styles.footerContainer}>
                <Text style={styles.previewText}>
                    Showing {previewItems.length} of {currentData.length} {activeTab === 'terms' ? 'terms & conditions' : 'promotions'}
                </Text>
                <TouchableOpacity
