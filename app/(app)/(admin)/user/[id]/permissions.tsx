...
export default function UserPermissionsScreen() {
  ...
  const [togglingPermissions, setTogglingPermissions] = useState<Set<string>>(
    new Set()
  );
  ...
  const currentUserPermissions = useMemo(() => {
    ...
    if (
      currentUser?.direct_permissions &&
      Array.isArray(currentUser.direct_permissions)
    ) {
  ...
  const permissionStats = useMemo(() => {
    ...
    const criticalCount = permissions.filter(
      p => p.is_critical && currentUserPermissions.includes(p.id)
    ).length;
    ...
    return {
      ...
      percentage:
        totalPermissions > 0
          ? Math.round((userPermCount / totalPermissions) * 100)
          : 0,
    };
        ...
        if (isCurrentlySelected) {
          ...
          if (permission?.dependencies) {
            for (const dep of permission.dependencies) {
              if (
                permissions.find(p => p.id === dep) &&
                !currentUserPermissions.includes(dep)
              ) {
  ...
  const getPermissionLevelIcon = (level: string) => {
    switch (level) {
      case 'read':
        return <Eye size={12} color={getPermissionLevelColor(level)} />;
      case 'write':
        return <Edit size={12} color={getPermissionLevelColor(level)} />;
      case 'delete':
        return <Trash2 size={12} color={getPermissionLevelColor(level)} />;
      case 'admin':
        return <Shield size={12} color={getPermissionLevelColor(level)} />;
      default:
        return <Eye size={12} color={getPermissionLevelColor(level)} />;
    }
  };
          ...
          >
            ...
            >
              <Settings
                size={16}
                color={
                  !selectedCategory ? colors.primary : colors.textSecondary
                }
              />
              <Text
                style={[
                      ...
                      <Text style={styles.categoryCount}>
                        {
                          categoryPermissions.filter(p =>
                            currentUserPermissions.includes(p.id)
                          ).length
                        }
                      ...
                      {categoryPermissions.map(permission => (
                        <View key={permission.id} style={styles.permissionRow}>
                          <Pressable
                            ...
                            } // Super admins have all permissions or currently toggling
                          >
