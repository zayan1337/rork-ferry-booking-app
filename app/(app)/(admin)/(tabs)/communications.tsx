...
export default function CommunicationsScreen() {
    ...
    const deliveredMessages = bulkMessages.reduce((sum, m) => sum + m.sent_count, 0);
    const failedMessages = bulkMessages.reduce((sum, m) => sum + m.failed_count, 0);
    const deliveryRate = totalRecipients > 0 ? Math.round((deliveredMessages / totalRecipients) * 100) : 0;
    const weeklyNotifications = notifications.filter(
        n => new Date(n.created_at) >= new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
    ).length;
    const emergencyNotifications = notifications.filter(n => n.type === "emergency" || n.priority === "critical").length;
            ...
            <View style={styles.sectionContent}>
                ...
                <View style={styles.itemsList}>
                    {notifications.slice(0, 10).map((notification) => {
                        ...
                        const timeAgo = Math.floor((Date.now() - sentTime.getTime()) / (1000 * 60));
                        const formattedTime = timeAgo < 60 ? `${timeAgo}m ago` :
                        ...
                        const estimatedReach = notification.target_users === "all" ? "All Users" :
                            notification.target_users === "customers" ? "All Customers" :
                            ...
                            <View key={notification.id} style={styles.notificationItem}>
                                <View style={styles.notificationHeader}>
                                        ...
                                        <View style={[styles.notificationTypeIcon, {
                                            backgroundColor: notification.type === "emergency" ? colors.danger + "20" :
                                                notification.type === "maintenance" ? colors.warning + "20" :
                                                    notification.type === "booking" ? "#34C759" + "20" :
                                                        notification.type === "payment" ? "#FF9500" + "20" :
                                                            colors.primary + "20"
                                        }]}>
                                    ...
                                    <View style={styles.notificationActions}>
                                        <View style={[styles.deliveryStatus, {
                                            backgroundColor: deliveryStatus === "delivered" ? "#34C759" + "20" : colors.danger + "20"
                                        }]}>
                                            <Text style={[styles.deliveryStatusText, {
                                                color: deliveryStatus === "delivered" ? "#34C759" : colors.danger
                                            }]}>{deliveryStatus}</Text>
                                        </View>
                                ...
                                <View style={styles.notificationMetrics}>
                                    <View style={styles.metricItem}>
                                        ...
                                        <Text style={styles.metricText}>Reach: {estimatedReach}</Text>
                                    </View>
                                    <View style={styles.metricItem}>
                                    ...
                                    <View style={styles.notificationSenderInfo}>
                                        ...
                                        <View>
                                            ...
                                            <Text style={styles.notificationDate}>
                                                {sentTime.toLocaleDateString()} at {sentTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                            </Text>
                                        </View>
                                    </View>
                                    <View style={styles.notificationTarget}>
                                        <Text style={styles.targetLabel}>Target:</Text>
                                        <Text style={styles.targetValue}>{notification.target_users}</Text>
                                    </View>
                                </View>
                            </View>
                        );
        ...
        return (
            <View style={styles.sectionContent}>
                <SectionHeader
                    ...
                    action={canSendBulkMessages() ? (
                        <View style={styles.headerActions}>
                            <Button
                                title="Analytics"
                                onPress={() => {/* View campaign analytics */ }}
                                size="small"
                                variant="outline"
                ...
                <View style={styles.itemsList}>
                    {bulkMessages.slice(0, 10).map((message) => {
                        ...
                        const timeAgo = Math.floor((Date.now() - createdTime.getTime()) / (1000 * 60));
                        const formattedTime = timeAgo < 60 ? `${timeAgo}m ago` :
                            timeAgo < 1440 ? `${Math.floor(timeAgo / 60)}h ago` :
                                `${Math.floor(timeAgo / 1440)}d ago`;

                        // Calculate delivery rate and engagement metrics
                        const deliveryRate = message.recipient_count > 0 ?
                            Math.round((message.sent_count / message.recipient_count) * 100) : 0;
                        const failureRate = message.recipient_count > 0 ?
                            Math.round((message.failed_count / message.recipient_count) * 100) : 0;

                        // Mock additional metrics
                        const openRate = message.status === "sent" ? Math.floor(Math.random() * 40) + 45 : 0;
                        const clickRate = message.status === "sent" ? Math.floor(Math.random() * 15) + 8 : 0;
                        const estimatedDeliveryTime = message.status === "sending" ? "2-3 min" :
                            message.status === "sent" ? "Completed" : "Pending";
                            ...
                            <View key={message.id} style={styles.bulkMessageItem}>
                                <View style={styles.bulkMessageHeader}>
                                    <View style={styles.bulkMessageTitleContainer}>
                                        ...
                                        <View style={styles.bulkMessageMeta}>
                                            <Text style={styles.bulkMessageId}>#{message.id.slice(-6)}</Text>
                                            <Text style={styles.bulkMessageTime}>{formattedTime}</Text>
                                        </View>
                                    </View>
                                    <View style={styles.bulkMessageActions}>
                                        <View style={[styles.statusBadge, {
                                            backgroundColor: message.status === "sent" ? "#34C759" + "20" :
                                                message.status === "sending" ? colors.warning + "20" :
                                                    message.status === "failed" ? colors.danger + "20" :
                                                        colors.textSecondary + "20"
                                        }]}>
                                            {message.status === "sent" && <CheckCircle size={12} color="#34C759" />}
                                            {message.status === "sending" && <Clock size={12} color={colors.warning} />}
                                            {message.status === "failed" && <XCircle size={12} color={colors.danger} />}
                                            {message.status === "draft" && <Edit size={12} color={colors.textSecondary} />}
                                            <Text style={[styles.statusText, {
                                                color: message.status === "sent" ? "#34C759" :
                                ...
                                <View style={styles.bulkMessageStats}>
                                    ...
                                    <View style={styles.statItem}>
                                        ...
                                        <Text style={styles.statText}>{message.sent_count} sent ({deliveryRate}%)</Text>
                                    </View>
                                    {message.failed_count > 0 && (
                                        <View style={styles.statItem}>
                                            <XCircle size={14} color={colors.danger} />
                                            <Text style={styles.statText}>{message.failed_count} failed ({failureRate}%)</Text>
                                        </View>
                                    )}
                                </View>
                                ...
                                {message.status === "sent" && (
                                    <View style={styles.engagementMetrics}>
                                        <View style={styles.engagementItem}>
                                            ...
                                            <Text style={styles.engagementText}>Open: {openRate}%</Text>
                                        </View>
                                        <View style={styles.engagementItem}>
                                            <Send size={14} color="#FF9500" />
                                            <Text style={styles.engagementText}>Click: {clickRate}%</Text>
                                        </View>
                                        <View style={styles.engagementItem}>
                                            <Clock size={14} color={colors.textSecondary} />
                                            <Text style={styles.engagementText}>Delivery: {estimatedDeliveryTime}</Text>
                                        </View>
                                    </View>
                                )}
                                    ...
                                    <View style={styles.messageSenderInfo}>
                                        ...
                                        <View>
                                            ...
                                            <Text style={styles.messageDate}>
                                                {createdTime.toLocaleDateString()} at {createdTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                            </Text>
                                        </View>
                                    </View>
        ...
        const templates = [
            {
                ...
                content: "Due to weather conditions, your trip has been delayed by {delay_time}. We apologize for the inconvenience.",
                category: "operations",
                usage_count: 45,
            ...
            },
            {
                ...
                content: "Your booking {booking_number} has been confirmed for {travel_date}. Thank you for choosing our ferry service.",
                category: "booking",
                usage_count: 238,
            ...
            },
            {
                ...
                content: "Please complete payment for booking {booking_number} before {due_date}. Payment can be made online or at the terminal.",
                category: "payment",
                usage_count: 67,
            ...
            },
            {
                ...
                content: "System maintenance scheduled from {start_time} to {end_time}. Some services may be temporarily unavailable.",
                category: "system",
                usage_count: 12,
            ...
            },
            {
                id: "5",
                name: "Weather Alert",
                content: "Weather alert: {weather_condition} expected. Trip {trip_id} may be affected. Please check for updates.",
                category: "emergency",
                usage_count: 23,
            ...
            },
            {
                ...
                content: "Welcome aboard! Your journey from {departure_port} to {arrival_port} will begin shortly.",
            ...
            <View style={styles.sectionContent}>
                ...
                <View style={styles.itemsList}>
                    {templates.map((template) => {
                        ...
                        const lastUsedText = daysSinceUsed === 0 ? "Today" :
                            daysSinceUsed === 1 ? "Yesterday" :
                                daysSinceUsed < 7 ? `${daysSinceUsed} days ago` :
                            ...
                            <View key={template.id} style={styles.templateItem}>
                                <View style={styles.templateHeader}>
                                    <View style={styles.templateTitleContainer}>
                                        ...
                                        <View style={styles.templateMeta}>
                                            <View style={[styles.templatePriority, {
                                                backgroundColor: template.priority === "critical" ? colors.danger + "20" :
                                                    template.priority === "high" ? colors.warning + "20" :
                                                        template.priority === "medium" ? colors.primary + "20" :
                                                            colors.textSecondary + "20"
                                            }]}>
                                                <Text style={[styles.templatePriorityText, {
                                                    color: template.priority === "critical" ? colors.danger :
                                                        template.priority === "high" ? colors.warning :
                                                            template.priority === "medium" ? colors.primary :
                                                                colors.textSecondary
                                                }]}>{template.priority}</Text>
                                            </View>
                                            <View style={[styles.templateCategory, {
                                                backgroundColor: template.category === "emergency" ? colors.danger + "20" :
                                                    template.category === "operations" ? colors.warning + "20" :
                                                        template.category === "booking" ? "#34C759" + "20" :
                                                            template.category === "payment" ? "#FF9500" + "20" :
                                                                colors.primary + "20"
                                            }]}>
                                                <Text style={[styles.templateCategoryText, {
                                                    color: template.category === "emergency" ? colors.danger :
                                                        template.category === "operations" ? colors.warning :
                                                            template.category === "booking" ? "#34C759" :
                                                                template.category === "payment" ? "#FF9500" :
                                                                    colors.primary
                                                }]}>{template.category.replace('_', ' ')}</Text>
                                            </View>
                                        </View>
                                    </View>
                                </View>
                                ...
                                <View style={styles.templateStats}>
                                    <View style={styles.templateStatItem}>
                                        ...
                                        <Text style={styles.templateStatText}>Used {template.usage_count} times</Text>
                                    </View>
                                    <View style={styles.templateStatItem}>
                                        <Clock size={14} color={colors.textSecondary} />
                                        <Text style={styles.templateStatText}>Last used: {lastUsedText}</Text>
                                    </View>
                                </View>
                                ...
                                <View style={styles.templateActions}>
                                    ...
                                    <TouchableOpacity style={styles.templateAction}>
                                        <Trash2 size={16} color={colors.danger} />
                                        <Text style={[styles.templateActionText, { color: colors.danger }]}>Delete</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                        );
