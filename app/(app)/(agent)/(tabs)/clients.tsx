...
export default function AgentClientsScreen() {
  ...
  const { agent, clients, bookings, isLoadingClients, error, refreshClients } =
    useAgentData();
  ...
  const getClientTotalBookingsCount = React.useCallback(
    (clientId: string) => {
  ...
  const getClientLastBookingDate = React.useCallback(
    (clientId: string) => {
  ...
  const { searchQuery, filteredClients, updateSearchQuery, searchStats } =
  ...
  const getInactiveBookingsCount = React.useCallback(
    (clientId: string) => {
      if (!bookings || bookings.length === 0) return 0;
  ...
  const renderClientItem = React.useCallback(
    ({ item }: { item: Client }) => {
    ...
    [
      handleClientPress,
      getInactiveBookingsCount,
      getClientLastBookingDate,
  ...
  const enhancedStats = {
    ...
    averageBookings:
      clientsWithCorrectCounts && clientsWithCorrectCounts.length > 0
        ? clientsWithCorrectCounts.reduce(
            (sum, client) => sum + (client.bookingsCount || 0),
            0
          ) / clientsWithCorrectCounts.length
        : 0,
  };
